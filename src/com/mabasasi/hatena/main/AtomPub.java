...
import com.mabasasi.hatena.data.UserProfile;
...
public class AtomPub {
     ...
     */
    public AtomPub(UserProfile profile) {
        this.profile = profile;
        
        this.rootEndPoint = BASE_URL  + profile.urlName + "/" + profile.blogId + "/atom";
    }
     ...
     * OAuth認証をする.
     * @return ユーザー情報
     * @throws exception.HatenaConnectionException はてなと正常接続出来なかった。
     */
    public static UserProfile HatenaAuthorization() throws HatenaConnectionException {
        try {
            // OAuth初期値の設定
            HatenaOauth oauth = new HatenaOauth(consumerKey, consumerSecret);
            
            // プロファイルの取得
            UserProfile profile = oauth.getUserJsonData();
            if (profile != null) {
                return profile;
            } else {
                // 個人データが取得できなかったら認証しなおす
                oauth.getAccessToken(null);
                profile = oauth.getUserJsonData();
                if (profile != null) {
                    oauth.saveProperties();
                    return profile;
                } else {
                    throw new HttpConnectionException("はてな からユーザー情報の取得に失敗しました。");
                }
            }
        } catch (OAuthException | IOException | URISyntaxException | UnexpectedValueException | HttpConnectionException ex) {
            throw new HatenaConnectionException(ex.getMessage());
        }
    }
